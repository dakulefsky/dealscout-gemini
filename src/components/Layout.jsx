...
import { ShoppingBag, LogOut, Settings, Heart, Search, X, Loader2 } from 'lucide-react';
...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    categoriesApi.list()
      ...
      .catch((error) => {
        if (error?.name !== 'AbortError') setCategoriesList([]);
      });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const results = await dealsApi.list(
          { q: query, status: 'APPROVED', limit: 6 },
    ...
    <div className="relative w-full">
      <form onSubmit={handleSearchSubmit}>
        <div className="relative">
          ...
          />
          {searchQuery && (
            <button type="button" onClick={clearSearch} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400" aria-label="Clear search">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
      ...
      {isSearchOpen && searchQuery.trim() && (
        <div className="absolute top-full left-0 right-0 mt-1.5 bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden z-50 p-2">
          {searchLoading ? (
            <div className="p-4 flex items-center justify-center gap-2 text-xs text-slate-500"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Searching…</div>
          ) : searchResults.length === 0 ? (
            <div className="p-4 text-center text-xs text-slate-500">No matching deals.</div>
          ) : (
            <div className="space-y-1">
              {searchResults.map((deal) => (
                <Link key={deal.id || deal.asin} to={`/deal/${deal.id || deal.asin}`} className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50">
                  <div className="w-10 h-10 rounded-lg bg-slate-50 border border-slate-100 p-1 shrink-0">
                    <Image src={deal.imageUrl} fallbackSrcs={deal.imageGallery || []} alt={deal.title} fittingType="contain" className="w-full h-full" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-semibold text-slate-900 truncate">{deal.title}</h4>
                    <div className="text-[11px] mt-0.5">
                      <span className="font-black text-emerald-700">${Number(deal.salePrice || 0).toFixed(2)}</span>
                      {deal.discountPercent > 0 && <span className="ml-2 text-emerald-700 font-semibold">{deal.discountPercent}% off</span>}
                    </div>
                  </div>
                </Link>
              ))}
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-200/90 bg-white/95 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-3 sm:gap-5">
          <Link to="/" className="flex items-center gap-2 font-heading font-black text-slate-900 text-lg sm:text-xl tracking-tight shrink-0">
            <div className="w-8 h-8 rounded-xl bg-emerald-600 text-white flex items-center justify-center"><ShoppingBag className="h-4 w-4" /></div>
          ...
          <nav className="flex items-center gap-1 sm:gap-2">
            <button type="button" onClick={() => setMobileSearchOpen((value) => !value)} className="md:hidden w-9 h-9 rounded-xl flex items-center justify-center text-slate-600 hover:bg-slate-100" title="Search"><Search className="h-4 w-4" /></button>
            <Link to="/saved" className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2 sm:px-3 py-2 rounded-xl ${location.pathname === '/saved' ? 'bg-rose-50 text-rose-600 border border-rose-200' : 'text-slate-600 hover:bg-slate-100'}`}>
              <Heart className={`h-4 w-4 ${savedDealIds.length ? 'text-rose-600 fill-rose-600' : 'text-slate-400'}`} />
            ...
            {isAuthenticated && user?.role === 'admin' && (
              <>
                <Link to="/admin" className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2 sm:px-3 py-2 rounded-xl ${location.pathname.startsWith('/admin') ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100'}`}><Settings className="h-4 w-4" /><span className="hidden sm:inline">Admin</span></Link>
                <button onClick={logout} className="inline-flex items-center gap-1 text-xs font-semibold text-slate-600 hover:bg-slate-100 px-2 py-2 rounded-xl" aria-label="Log out"><LogOut className="h-4 w-4" /></button>
              </>
            )}
          </nav>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 sm:py-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div>
              ...
              <ul className="space-y-1.5 text-xs">{categoriesList.slice(0, 5).map((category) => <li key={category.id}><Link to={`/category/${category.slug}`} className="text-slate-600 hover:text-emerald-600">{category.name}</Link></li>)}</ul>
            </div>
            <div>
