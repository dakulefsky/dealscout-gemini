      ...
      <div className="space-y-3.5 max-h-72 overflow-y-auto pr-1 mb-6 text-sm">
        {messages.map((m, idx) => (
          <div key={idx} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`p-3.5 rounded-2xl max-w-[85%] leading-relaxed ${m.role === 'user' ? 'bg-emerald-600 text-white rounded-br-none' : 'bg-slate-800/90 text-slate-200 border border-slate-700/80 rounded-bl-none prose prose-invert prose-sm'}`}>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {quickPrompts.map((prompt, i) => (
            <button key={i} type="button" onClick={() => handleSend(prompt)} disabled={isLoading} className="text-xs bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white px-3 py-1.5 rounded-xl border border-slate-700/80 transition text-left">
              {prompt}
            </button>
          ))}
        </div>
