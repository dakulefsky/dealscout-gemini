 ...
 * 
 * Formats:
 * - 'sidebar-rectangle': 300x250 (Medium Rectangle) - for sidebar placement
 * - 'half-page': 300x600 (Large Skyscraper / Half Page) - for tall sidebar placement
 * - 'in-content': Responsive horizontal banner (min-h-[140px] to min-h-[250px]) - between product specs & reviews
 * - 'leaderboard': 728x90 (Responsive Leaderboard) - top/bottom product page banners
 * - 'responsive': Auto-fluid responsive container with min-h-[250px]
 */
export default function AdSensePlaceholder({
  ...
  // When live AdSense script is present in index.html, trigger adsbygoogle push safely
  useEffect(() => {
    try {
  ...
  // Format configurations with strict dimensions to reserve layout space
  const formatConfigs = {
    'sidebar-rectangle': {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider px-1">
        ...
        <span className="text-[9px] font-semibold text-slate-400 bg-white/80 border border-slate-200/70 px-1.5 py-0.5 rounded shadow-2xs">
          Google AdSense
        </span>
      </div>

      {/* Reserved Slot Body / AdSense Container */}
      <div className="flex-1 flex flex-col items-center justify-center my-2 text-center p-3 rounded-xl border border-dashed border-slate-300/80 bg-white/70">
        {/* Real AdSense tag ready for production script */}
        <ins
          ref={adRef}
        ...
        {/* Visual Fallback / Reserved Geometry Indicator */}
        <div className="flex flex-col items-center justify-center gap-1.5 text-slate-400">
          ...
          <span className="text-xs font-bold text-slate-600">
            AdSense Ad Space Reserved
          </span>
          <span className="text-[11px] text-slate-400 font-medium">
            {currentConfig.dimensionText}
          </span>
        </div>
      </div>
