...
export default function DealCard({ deal, viewMode = 'grid' }) {
  ...
  if (viewMode === 'list') {
    return (
      <Link to={`/deal/${dealId}`} className={`group rounded-2xl border p-3 sm:p-4 transition-all flex gap-3 sm:gap-4 items-center relative w-full ${isExpired ? 'bg-slate-50/80 border-dashed border-slate-300 opacity-75' : 'bg-white border-slate-200 hover:shadow-md hover:border-slate-300'}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            {!isExpired && deal.category && <span className="text-[9px] uppercase font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded truncate max-w-[150px]">{deal.category}</span>}
            {isExpired && <span className="text-[9px] font-bold text-amber-800 bg-amber-100 px-1.5 py-0.5 rounded"><AlertCircle className="w-2.5 h-2.5 inline" /> {hoursLeft ? `Deletes in ${hoursLeft}h` : 'Ended'}</span>}
            {sourceBadge}
          </div>
          ...
          <div className="flex items-baseline gap-2 mt-2">
            <span className={`text-lg sm:text-xl font-black ${isExpired ? 'text-slate-500 line-through' : 'text-emerald-700'}`}>{formatPrice(deal.salePrice)}</span>
            {deal.originalPrice > deal.salePrice && <span className="text-xs text-slate-400 line-through">{formatPrice(deal.originalPrice)}</span>}
            {!isExpired && savings > 0 && <span className="hidden sm:inline text-[10px] font-bold text-emerald-700">Save {formatPrice(savings)}</span>}
          </div>
        </div>
  ...
  return (
    <Link to={`/deal/${dealId}`} className={`group rounded-xl sm:rounded-2xl border overflow-hidden transition-all flex flex-col h-full w-full relative ${isExpired ? 'bg-slate-50/90 border-dashed border-slate-300 opacity-75' : 'bg-white border-slate-200 hover:shadow-md hover:border-slate-300'}`}>
      <div className={`relative aspect-[4/3] w-full overflow-hidden p-2.5 sm:p-4 border-b ${isExpired ? 'bg-slate-100 border-slate-200 grayscale-[0.85]' : 'bg-slate-50/70 border-slate-100'}`}>
        ...
        {isExpired ? (
          <span className="absolute top-2 left-2 bg-slate-800 text-white text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-md"><Clock className="w-2.5 h-2.5 inline mr-1 text-amber-400" /> Ended</span>
        ) : deal.discountPercent > 0 ? (
          <span className="absolute top-2 left-2 inline-flex items-center gap-1 bg-emerald-600 text-white text-[10px] sm:text-[11px] font-black px-1.5 sm:px-2 py-0.5 rounded-md"><TrendingDown className="w-3 h-3 hidden sm:block" /> {deal.discountPercent}% OFF</span>
        ) : null}
      ...
      <div className="p-2.5 sm:p-4 flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between gap-1.5 min-w-0 mb-2">
          {isExpired ? <span className="text-[9px] font-bold text-amber-800 bg-amber-100 px-1.5 py-0.5 rounded truncate">{hoursLeft ? `Deletes in ${hoursLeft}h` : 'Ended'}</span> : <span className="text-[9px] sm:text-[10px] uppercase font-bold text-emerald-700 bg-emerald-50 px-1.5 sm:px-2 py-0.5 rounded truncate max-w-[95px] sm:max-w-[130px]">{deal.category || 'Deal'}</span>}
          {sourceBadge}
        </div>
        ...
        <div className="mt-auto pt-2.5 sm:pt-3 border-t border-slate-100 flex items-end justify-between gap-1.5 min-w-0">
          <div className="min-w-0">
            <div className="flex items-baseline gap-1 min-w-0">
              <span className={`text-base sm:text-lg font-black truncate ${isExpired ? 'text-slate-500 line-through' : 'text-emerald-700'}`}>{formatPrice(deal.salePrice)}</span>
              {deal.originalPrice > deal.salePrice && <span className="text-[10px] sm:text-xs text-slate-400 line-through truncate">{formatPrice(deal.originalPrice)}</span>}
            </div>
            {!isExpired && savings > 0 && <div className="text-[9px] sm:text-[10px] font-semibold text-emerald-700 mt-0.5">Save {formatPrice(savings)}</div>}
          </div>
          <span className="hidden sm:inline-flex text-xs font-bold items-center gap-1 shrink-0 text-slate-500 group-hover:text-emerald-600">View <ArrowRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" /></span>
        </div>
      </div>
    </Link>
  );
