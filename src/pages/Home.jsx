...
export default function Home() {
  ...
  const visibleDeals = useMemo(() => {
    let list = deals.filter((deal) => !dismissals[dealIdentity(deal)]);
  ...
  }, [deals, dismissals, activeCat, searchQuery, minDiscount, priceTier, sort, interests]);
  ...
  const newSinceLastVisit = lastVisit > 0 ? deals.filter((deal) => !dismissals[dealIdentity(deal)] && dealTimestamp(deal) > lastVisit).length : 0;
