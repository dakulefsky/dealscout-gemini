  ...
  return (
    <div className="ds-shell py-8 sm:py-12 pb-20">
      <header className="border-y border-emerald-950/10 py-7 sm:py-10 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <div>
          <div className="ds-kicker inline-flex items-center gap-1.5"><Heart className="w-3.5 h-3.5 fill-emerald-800" /> Saved</div>
          <h1 className="font-heading text-4xl sm:text-5xl font-bold text-emerald-950 mt-2">Saved deals</h1>
          <p className="text-sm text-slate-600 mt-3 max-w-xl">Deals you marked to check again.</p>
      ...
      <main className="pt-8 sm:pt-10">
        {isLoading ? (
          ...
          <section className="max-w-3xl mx-auto py-16 sm:py-24 text-center border-y border-emerald-950/10">
            ...
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-emerald-950 mt-2">Nothing saved yet.</h2>
            <p className="text-sm text-slate-500 leading-relaxed mt-4 max-w-md mx-auto">Use the heart on any deal to keep it here.</p>
            <Link to="/" className="mt-7 inline-flex items-center gap-2 bg-emerald-950 text-white px-5 py-3 text-sm font-bold hover:bg-emerald-900">Browse deals <ArrowRight className="w-4 h-4" /></Link>
          </section>
        ) : (
