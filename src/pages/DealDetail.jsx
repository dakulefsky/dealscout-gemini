...
export default function DealDetail() {
  ...
  return (
    <div className="ds-shell py-6 sm:py-9 pb-28 lg:pb-14">
      ...
      {recommendations.length > 0 && <section aria-labelledby="recommended-deals-heading" className="mt-14 sm:mt-16 pt-7 sm:pt-9 border-t border-emerald-950/15"><div className="flex items-end justify-between gap-5 mb-5 sm:mb-6"><div><div className="ds-kicker">Keep browsing</div><h2 id="recommended-deals-heading" className="font-heading text-2xl sm:text-3xl font-bold text-emerald-950 mt-1">Other live deals</h2></div><Link to="/?category=all" className="hidden sm:inline-flex shrink-0 items-center gap-1 text-xs font-bold text-emerald-900 border-b border-emerald-900 pb-1">See all deals <ArrowRight className="w-3.5 h-3.5" /></Link></div><div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 auto-rows-fr items-stretch">{recommendations.map((item) => <DealCard key={dealIdentity(item)} deal={item} />)}</div></section>}
