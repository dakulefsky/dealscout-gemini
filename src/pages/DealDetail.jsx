...
function rankRecommendations(items, currentDeal) {
  ...
  return personalizedRank(eligible, interests)
...
export default function DealDetail() {
  ...
  ].filter(Boolean), [deal.discountPercent, deal.sourceVerified, freshness.label, freshness.stale, savings]);
    ...
    <div className="ds-shell py-6 sm:py-9 pb-28 lg:pb-14">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.55fr)_minmax(340px,0.9fr)] gap-8 lg:gap-12 pt-7 sm:pt-10 items-start">
        <main className="min-w-0">
          ...
          {dealFacts.length > 0 && (
            <section aria-label="Deal facts" className="grid grid-cols-1 sm:grid-cols-3 border-x border-b border-emerald-950/10 bg-white">
              {dealFacts.map(({ label, value, icon: Icon }, index) => (
                <div key={label} className={`px-4 py-4 flex items-center gap-3 ${index > 0 ? 'sm:border-l border-emerald-950/10' : ''}`}>
                  <Icon className="w-4 h-4 text-emerald-800 shrink-0" />
                  <div className="min-w-0"><div className="text-[9px] uppercase tracking-[0.14em] font-black text-slate-400">{label}</div><div className="text-sm font-black text-emerald-950 truncate mt-0.5">{value}</div></div>
                </div>
              ))}
            </section>
          )}
        ...
        <aside className="lg:sticky lg:top-24 min-w-0">
          ...
          <div className="mt-7 border-y border-emerald-950/10 py-5">
            ...
            {!deal.isExpired && savings > 0 && <div className="mt-3 flex items-center justify-between gap-3"><span className="text-sm font-bold text-emerald-800">You save {formatPrice(savings)}</span>{deal.discountPercent > 0 && <span className="text-xs font-black bg-[#dcebdc] text-emerald-950 px-2 py-1">{deal.discountPercent}% OFF</span>}</div>}
          </div>
      ...
      {recommendations.length > 0 && (
        <section aria-labelledby="recommended-deals-heading" className="mt-14 sm:mt-18 pt-7 sm:pt-9 border-t border-emerald-950/15">
          <div className="flex items-end justify-between gap-5 mb-5 sm:mb-6">
            <div>
              <div className="ds-kicker">Keep browsing</div>
              <h2 id="recommended-deals-heading" className="font-heading text-3xl sm:text-4xl font-bold text-emerald-950 mt-1">More deals you might like</h2>
              <p className="text-xs sm:text-sm text-slate-500 mt-2 max-w-2xl">Live DealScout finds, weighted toward this category and what you tend to save or open.</p>
            </div>
            <Link to="/?category=all" className="hidden sm:inline-flex shrink-0 items-center gap-1 text-xs font-bold text-emerald-900 border-b border-emerald-900 pb-1">See all deals <ArrowRight className="w-3.5 h-3.5" /></Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 auto-rows-fr items-stretch">
            {recommendations.map((item) => <DealCard key={dealIdentity(item)} deal={item} />)}
          </div>
        </section>
      )}
