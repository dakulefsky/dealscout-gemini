...
function createDealScoutClient({ baseUrl = '', fetchImpl = globalThis.fetch, getToken = async () => null, getGuestId = async () => null, defaultTimeoutMs = DEFAULT_TIMEOUT_MS, timers } = {}) {
  ...
  const functions = {
    ...
    verifyPrices: (limit = 15) => api.post('/api/functions/verify-prices', { limit }, { timeoutMs: MAINTENANCE_TIMEOUT_MS }),
    ...
    setProviderApiEnabled: (enabled) => api.post('/api/functions/channel-settings', { providerApiEnabled: Boolean(enabled) }),
    jewishCalendar: () => api.get('/api/functions/jewish-calendar'),
