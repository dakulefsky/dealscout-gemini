...
test('core shopping routes stay eager while admin and secondary routes remain lazy loaded', () => {
  assert.match(app, /import Home from '@\/pages\/Home'/);
  assert.match(app, /import DealDetail from '@\/pages\/DealDetail'/);
  ...
  assert.doesNotMatch(app, /const DealDetail = lazy/);
