...
test('publication worker production role does not require website-only secrets', () => {
  const errors = validateProductionRuntime({}, { postgresConfigured: true, role: RUNTIME_ROLES.PUBLICATION_WORKER });
  assert.deepEqual(errors, []);
  const webErrors = validateProductionRuntime({}, { postgresConfigured: true, role: RUNTIME_ROLES.WEB });
...
test('publication worker configuration is explicit, bounded and HTTPS in production', () => {
  const config = resolvePublicationWorkerConfig({ NODE_ENV: 'production', PUBLICATION_CHANNEL: 'whatsapp_status', PUBLICATION_TRANSPORT: 'webhook', PUBLICATION_RUN_MODE: 'once', PUBLICATION_WEBHOOK_URL: 'https://publisher.example/hook', PUBLICATION_WEBHOOK_TOKEN: '0123456789abcdef', PUBLICATION_POLL_MS: '5000', PUBLICATION_QUEUE_BATCH: '3', PUBLICATION_MAX_PER_CYCLE: '2' }, { isProduction: true });
  ...
  assert.throws(() => resolvePublicationWorkerConfig({ PUBLICATION_CHANNEL: 'whatsapp_status', PUBLICATION_TRANSPORT: 'webhook', PUBLICATION_WEBHOOK_URL: 'http://publisher.example/hook', PUBLICATION_WEBHOOK_TOKEN: '0123456789abcdef' }, { isProduction: true }), /https/);
});

test('publication cycle feeds the queue before bounded Status draining', async () => {
  const calls = { query: null, queue: null, runs: 0 };
  const candidates = [{ asin: 'B000000001', discount_percent: 30 }, { asin: 'B000000002', discount_percent: 25 }];
  const result = await runPublicationCycle({ channel: 'whatsapp_status', candidateLimit: 50, queueBatch: 3, maxPublishesPerCycle: 4, minPublishSpacingSeconds: 0 }, { publish: async () => ({}) }, {
    publicationMetrics: { latestPublishedAt: async () => null },
    postgres: { withAdvisoryLock: async (_id, task) => ({ acquired: true, result: await task() }) },
    dealQueries: { async list(options) { calls.query = options; return candidates; } },
    publication: { async queueBestDeals(channel, rows, options) { calls.queue = { channel, rows, options }; return { selectedCount: 2, createdCount: 2, jobs: [] }; } },
    worker: { async runPublicationOnce() { calls.runs += 1; return { status: 'published', channel: 'whatsapp_status', jobId: 'job-1' }; } },
  });
  assert.equal(calls.query.minDiscount, 25);
...
test('WhatsApp Status defers transport until durable spacing has elapsed', async () => {
  ...
  const result = await runPublicationCycle({ channel: 'whatsapp_status', candidateLimit: 50, queueBatch: 2, maxPublishesPerCycle: 5, minPublishSpacingSeconds: 1800 }, { publish: async () => ({}) }, {
    nowUnix: () => now,
...
test('WhatsApp Status cycle defers when another replica owns the publication lock', async () => {
  let queried = false;
  const result = await runPublicationCycle({ channel: 'whatsapp_status', candidateLimit: 50, queueBatch: 2, maxPublishesPerCycle: 1, minPublishSpacingSeconds: 1800 }, { publish: async () => ({}) }, {
...
test('webhook transport sends a versioned idempotent envelope and returns external identity', async () => {
  let request;
  const adapter = createWebhookPublicationAdapter({ webhookUrl: 'https://publisher.example/hook', webhookToken: 'secret-token-value', webhookTimeoutMs: 5000 }, { fetchImpl: async (url, options) => { request = { url, options }; return { ok: true, status: 200, headers: { get: (name) => name.toLowerCase() === 'content-type' ? 'application/json' : null }, json: async () => ({ externalPublicationId: 'wa-123' }) }; } });
  const result = await adapter.publish({ channel: 'whatsapp_status', job: { id: 'job-1', idempotency_key: 'idem-1', asin: 'B000000001', source_price_check_at: 123 }, content: { channel: 'whatsapp_status', facts: { asin: 'B000000001' }, caption: 'Deal', format: 'image_caption' } });
