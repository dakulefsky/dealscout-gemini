...
function getClientIdentity(req) {
  ...
  if (authHeader?.startsWith('Bearer ') && JWT_SECRET && JWT_SECRET.length >= 32) {
    try {
      ...
      const user = db.tables.users.find((u) => u.id === decoded.id);
...
router.get('/', (req, res) => {
  ...
  const savedDeals = db.tables.deals
    .filter((d) => publicDeal(d) && (dealIds.includes(d.id) || dealIds.includes(d.asin)))
    .map((deal) => {
      const bookmark = userBookmarks.find((b) => b.dealId === deal.id || b.dealId === deal.asin);
      return { ...rowToDeal(deal), savedAt: bookmark?.createdAt || null, targetPrice: bookmark?.targetPrice || null };
    });
...
router.post('/toggle', (req, res) => {
  ...
  const deal = db.tables.deals.find((d) => (d.id === dealId || d.asin === dealId) && publicDeal(d));
