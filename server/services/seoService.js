...
function buildSitemap({ baseUrl, deals = [], categories = [] }) {
  const urls = [
    ...
    ...deals.map((d) => ({ loc: `${baseUrl}/deal/${encodeURIComponent(d.id || d.asin)}`, priority: '0.9', changefreq: 'hourly', lastmod: d.price_check_at ? new Date(Number(d.price_check_at) * 1000).toISOString() : undefined })),
...
function replaceMeta(html, { title, description, canonical, robots = 'index,follow', jsonLd }) {
  let out = html;
...
function dealMeta(baseUrl, deal) {
  ...
  const description = `${deal.discount_percent || 0}% off${savings > 0 ? `, save $${savings.toFixed(2)}` : ''}. Price recently checked on DealScout; confirm final price and availability on Amazon.`;
...
module.exports = { siteBase, buildSitemap, buildRobots, replaceMeta, homeMeta, categoryMeta, dealMeta };
